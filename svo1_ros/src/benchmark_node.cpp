...
#include <svo1_ros/dataset_img.h>
...
BenchmarkNode::BenchmarkNode(ros::NodeHandle& nh) :
    vo_(NULL),
    frame_count_(0),
    img_noise_sigma_(vk::getParam<double>("svo1/dataset_noise_sigma", 0.0))
{
  // Create Camera
...
void BenchmarkNode::runBenchmark(const std::string& dataset_dir)
{
  ...
  if(!dataset_reader.next()) {
    SVO1ERROR_STREAM("Failed opening dataset: "<<filename_benchmark);
    return;
  }
  ...
  // process dataset
  int first_frame = vk::getParam<int>("svo1/dataset_first_frame", 0);
...
void BenchmarkNode::runBlenderBenchmark(const std::string& dataset_dir)
{
  ...
  for(auto it = dataset.begin(); it != dataset.end() && ros::ok(); ++it, ++frame_count_)
  {
    ...
    if(frame_count_ == 0)
    {
      ...
      svo1::feature_detection::FastDetector detector(
          cam_->width(), cam_->height(), svo1::Config::gridSize(), svo1::Config::nPyrLevels());
      detector.detect(frame_ref.get(), frame_ref->img_pyr_, svo1::Config::triangMinCornerScore(), frame_ref->fts_);
      std::for_each(frame_ref->fts_.begin(), frame_ref->fts_.end(), [&](Feature* ftr) {
        ...
        svo1::Point* point = new svo1::Point(pt_pos_world, ftr);
        ftr->point = point;
      });
      SVO1INFO_STREAM("Added "<<frame_ref->nObs()<<" 3d pts to the reference frame.");
      vo_->setFirstFrame(frame_ref);
      SVO1INFO_STREAM("Set reference frame.");
    }
    else
    {
      SVO1DEBUG_STREAM("Processing image "<<it->image_name_<<".");
      vo_->addImage(img, it->timestamp_);
      visualizer_.publishMinimal(img, vo_->lastFrame(), *vo_, it->timestamp_);
      visualizer_.visualizeMarkers(vo_->lastFrame(), vo_->coreKeyframes(), vo_->map());
    }
    ...
    if(vo_->stage() != svo1::FrameHandlerMono::STAGE_DEFAULT_FRAME)
    {
      SVO1ERROR_STREAM("SVO failed before entire dataset could be processed.");
      break;
    }
...
int main(int argc, char** argv)
{
  ...
  ros::NodeHandle nh;
  svo1::BenchmarkNode benchmark(nh);
  std::string benchmark_dir(vk::getParam<std::string>("svo1/dataset_directory"));
  if(vk::getParam<bool>("svo1/dataset_is_blender", false))
