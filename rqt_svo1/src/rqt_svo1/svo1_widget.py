...
from svo1_msgs.msg import Info
from std_msgs.msg import String

class Svo1Widget(QWidget):
  _last_info_msg = Info()
  _publisher = None
  _subscriber = None
  _num_received_msgs = 0
  _svo1_namespace = None
  def __init__(self, svo1_namespace='svo1'):
    ...
    # load UI
    ui_file = os.path.join(rospkg.RosPack().get_path('rqt_svo1'), 'resource', 'widget.ui')
    loadUi(ui_file, self)
  ...
  def _on_topic_changed(self, topic):
    self._svo1_namespace = str(topic)
    self.unregister()
    self.register(self._svo1_namespace)

  def register(self, svo1_namespace):
    # Load parameters
    max_num_features = rospy.get_param(svo1_namespace+'/max_fts', 120)
    ...
    self._subscriber = rospy.Subscriber(svo1_namespace+'/info', Info, self.info_cb)

    # Initialize Publisher
    self._publisher = rospy.Publisher(svo1_namespace+'/remote_key', String)
  ...
  def update_info(self):
    ...
    # set info text
    self.svo1_info_label.setText(info_text)
