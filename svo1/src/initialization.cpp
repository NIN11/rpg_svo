...
InitResult KltHomographyInit::addFirstFrame(FramePtr frame_ref)
{
  ...
  if(px_ref_.size() < 100)
  {
    SVO1WARN_STREAM_THROTTLE(2.0, "First image has less than 100 features. Retry in more textured environment.");
    return FAILURE;
  }
...
InitResult KltHomographyInit::addSecondFrame(FramePtr frame_cur)
{
  ...
  SVO1INFO_STREAM("Init: KLT tracked "<< disparities_.size() <<" features");

  if(disparities_.size() < Config::initMinTracked())
    return FAILURE;

  double disparity = vk::getMedian(disparities_);
  SVO1INFO_STREAM("Init: KLT "<<disparity<<"px average disparity.");
  ...
  SVO1INFO_STREAM("Init: Homography RANSAC "<<inliers_.size()<<" inliers.");

  if(inliers_.size() < Config::initMinInliers())
  {
    SVO1WARN_STREAM("Init WARNING: "<<Config::initMinInliers()<<" inliers minimum required.");
    return FAILURE;
  }
