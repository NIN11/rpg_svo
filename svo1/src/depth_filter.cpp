...
{
  stopThread();
  SVO1INFO_STREAM("DepthFilter destructed.");
}
...
{
  SVO1INFO_STREAM("DepthFilter stop thread invoked.");
  if(thread_ != NULL)
  {
    SVO1INFO_STREAM("DepthFilter interrupt and join thread... ");
...
void DepthFilter::initializeSeeds(FramePtr frame)
{
  ...
  if(options_.verbose)
    SVO1INFO_STREAM("DepthFilter: Initialized "<<new_features.size()<<" new seeds");
  seeds_updating_halt_ = false;
}
...
void DepthFilter::reset()
{
  ...
  if(options_.verbose)
    SVO1INFO_STREAM("DepthFilter: RESET.");
}
