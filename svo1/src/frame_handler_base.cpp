...
#include <fstream>
#include <svo1/frame_handler_base.h>
  ...
  tracking_quality_(TRACKING_INSUFFICIENT)
{
  ...
  SVO1INFO_STREAM("SVO initialized");
}

FrameHandlerBase::~FrameHandlerBase()
{
  SVO1INFO_STREAM("SVO destructor invoked");
#ifdef SVO1TRACE
  delete g_permon;
#endif
}
...
bool FrameHandlerBase::startFrameProcessingCommon(const double timestamp)
{
  ...
  SVO1LOG(timestamp);
  SVO1DEBUG_STREAM("New Frame");
  SVO1START_TIMER("tot_time");
  timer_.start();
...
{
  SVO1DEBUG_STREAM("Frame: "<<update_id<<"\t fps-avg = "<< 1.0/acc_frame_timings_.getMean()<<"\t nObs = "<<acc_num_obs_.getMean());
  SVO1LOG(dropout);
...
void FrameHandlerBase::setTrackingQuality(const size_t num_observations)
{
  ...
  {
    SVO1WARN_STREAM_THROTTLE(0.5, "Tracking less than "<< Config::qualityMinFts() <<" features!");
    tracking_quality_ = TRACKING_INSUFFICIENT;
  }
  ...
  {
    SVO1WARN_STREAM("Lost "<< feature_drop <<" features!");
    tracking_quality_ = TRACKING_INSUFFICIENT;
  }
}
