...
class DepthFilterTest {
 public:
  ...
  void depthFilterCb(svo1::Point* point, double depth_sigma2);
  ...
  svo1::DepthFilter* depth_filter_;
...
void DepthFilterTest::depthFilterCb(svo1::Point* point, double depth_sigma2)
{
    ...
    std::string experiment_name)
{
  ...
  svo1::feature_detection::DetectorPtr feature_detector(
      new svo1::feature_detection::FastDetector(
          cam_->width(), cam_->height(), svo1::Config::gridSize(), svo1::Config::nPyrLevels()));
  svo1::DepthFilter::callback_t depth_filter_cb = boost::bind(&DepthFilterTest::depthFilterCb, this, _1, _2);
  depth_filter_ = new svo1::DepthFilter(feature_detector, depth_filter_cb);
  ...
  for(int i=0; it != sequence.end() && i<20; ++it, ++i)
  {
    ...
    if(i == 0)
    {
      ...
      frame_ref_ = boost::make_shared<svo1::Frame>(cam_, img, 0.0);
    ...
    n_converged_seeds_ = 0;
    frame_cur_ = boost::make_shared<svo1::Frame>(cam_, img, 0.0);
