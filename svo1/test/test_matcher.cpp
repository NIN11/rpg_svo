...
class MatcherTest {
 public:
  MatcherTest()
  {
    ...
    // create frame
    frame_ref_ = new svo1::Frame(cam_, img_ref, 1.0);
    frame_cur_ = new svo1::Frame(cam_, img_cur, 2.0);
    ref_ftr_ = new svo1::Feature(frame_ref_, Eigen::Vector2d(300, 260), 0);
  ...
  svo1::Frame* frame_ref_;
  svo1::Frame* frame_cur_;
  svo1::Feature* ref_ftr_;
  cv::Mat depth_ref_;
};

void MatcherTest::testEpipolarSearchFullImg()
{
  svo1::Matcher matcher;
  ...
  for(int y=4; y<depthmap.rows-4; ++y)
  {
    ...
    {
      svo1::Feature ftr(frame_ref_, Eigen::Vector2d(x,y), 0);
...
void MatcherTest::testWarpAffine()
{
  ...
  svo1::warp::getWarpMatrixAffine(
      *cam_, *cam_, ref_ftr_->px, ref_ftr_->f, 1.0, T_cur_ref, ref_ftr_->level, A_cur_ref);
  int level_cur = svo1::warp::getBestSearchLevel(A_cur_ref, svo1::Config::nPyrLevels()-1);
  svo1::warp::warpAffine(
