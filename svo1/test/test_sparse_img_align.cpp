...
class SparseImgAlignTest {
 public:
  ...
  void testSequence(
      ...
      svo1::feature_detection::AbstractDetector* feature_detector);

  vk::PinholeCamera* cam_;
  svo1::FramePtr frame_ref_;
  svo1::FramePtr frame_cur_;
};
    ...
    svo1::feature_detection::AbstractDetector* feature_detector)
{
  ...
  std::string trace_dir(svo1::test_utils::getTraceDir());
  ...
  for(int i=0; iter != sequence.end() && i<30; ++iter, ++i)
  {
    ...
    {
      // set reference frame
      frame_ref_.reset(new svo1::Frame(cam_, img, 0.0));
      ...
      feature_detector->detect(
          frame_ref_.get(), frame_ref_->img_pyr_, svo1::Config::triangMinCornerScore(), frame_ref_->fts_);
      std::for_each(frame_ref_->fts_.begin(), frame_ref_->fts_.end(), [&](svo1::Feature* i) {
        ...
        svo1::Point* pt = new svo1::Point(pt_pos_w, i);
        i->point = pt;
      });
    ...
    frame_cur_.reset(new svo1::Frame(cam_, img, 0.0));
    //frame_cur_->T_f_w_ = frame_ref_->T_f_w_; // start at reference frame
    frame_cur_->T_f_w_ = T_prev_w; // start at last frame

    // run image align
    vk::Timer t;
    svo1::SparseImgAlign img_align(svo1::Config::kltMaxLevel(), svo1::Config::kltMinLevel(),
                                    30, svo1::SparseImgAlign::GaussNewton, false, false);
...
int main(int argc, char** argv)
{
  ...
  std::string dataset_dir(svo1::test_utils::getDatasetDir() + "/sin2_tex2_h1_v8_d");
  svo1::Config::triangMinCornerScore() = 20;
  svo1::Config::kltMinLevel() = 0;
  SparseImgAlignTest test;
  svo1::feature_detection::FastDetector detector(
      test.cam_->width(), test.cam_->height(), svo1::Config::gridSize(), svo1::Config::nPyrLevels());
