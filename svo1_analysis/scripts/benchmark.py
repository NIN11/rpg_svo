...
import argparse
import time
import vikit1_py.cpu_info as cpu_info
import vikit1_py.ros_node as ros_node
import evaluate
import shutil

def run_experiment(dataset, params):
    # load dataset parameters
    params['dataset_directory'] = os.path.join(os.environ['SVO1DATASET_DIR'], dataset)
    ...
    algo_params_file = os.path.join(rospkg.RosPack().get_path('svo1_ros'),
...
if __name__=="__main__":
    ...
    ''')
    parser.add_argument('experiment_file', help='experiment file in svo1_analysis/experiments folder')
    parser.add_argument('--evaluate', help='evaluate tracefile after running SVO1, action='store_true')
    parser.add_argument('--version', help='version of svo1 to evaluate', default='svo1_ros')
    ...
    experiment_params_file = os.path.join(rospkg.RosPack().get_path('svo1_analysis'),
    ...
    for dataset in experiment_params['datasets']:
        ...
        params['trace_dir'] = os.path.join(rospkg.RosPack().get_path('svo1_analysis'),
